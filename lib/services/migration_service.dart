...
class MigrationService {
  ...
  static Future<Uint8List> migrate(String masterPassword, String email) async {
    ...
    for (final row in vaultRows) {
      try {
        final plaintext = CryptoService.decrypt(row['encrypted_data'] as String, legacyKey);
        ...
        // Entry may already be migrated or corrupted, skip
      }
    }
  ...
  static bool verifyPassword(Uint8List key) {
    final meta = supabase.auth.currentUser?.userMetadata;
    if (meta == null || meta['key_check'] == null) return true; // Legacy, can't verify
